...
try {
  ...
  process.exitCode = mean(results.map((result) => result.recall)) >= PASS_THRESHOLD ? 0 : 1;
} finally {
  graph.close();
...
function parseRunnerArgs(args: string[]): RunnerOptions {
  const fresh = args.includes('--fresh') || Bun.env.ASTROGRAPH_EVAL_FRESH === '1';
  const positionals = args.filter((arg) => arg !== '--fresh');
  return {
    repoPath: resolvePath(positionals[0] ?? Bun.env.ASTROGRAPH_EVAL_REPO ?? '.'),
    fresh,
  };
}
...
function printReport(repoPath: string, results: EvalResult[]): void {
  ...
  console.log('');
  console.log('case                         status  recall  mrr    ms      missed');
  console.log('---------------------------  ------  ------  -----  ------  ----------------');
...
function tmpBaseDir(): string {
  return (Bun.env.TMPDIR ?? Bun.env.TEMP ?? Bun.env.TMP ?? '/tmp').replaceAll('\\', '/').replace(/\/$/, '');
}
