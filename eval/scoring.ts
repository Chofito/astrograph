...
export function scoreSearch(evalCase: EvalCase, results: SearchOutput, latencyMs: number): EvalResult {
  return scoreNodeList(evalCase, results.map((result) => result.node), latencyMs);
}

export function scoreContext(evalCase: EvalCase, context: ContextOutput, latencyMs: number): EvalResult {
...
function scoreNodeList(evalCase: EvalCase, nodes: NodeRef[], latencyMs: number): EvalResult {
  ...
  const recall = evalCase.expectedSymbols.length > 0 ? found.length / evalCase.expectedSymbols.length : 0;
...
function reciprocalRank(expectedSymbols: string[], nodes: NodeRef[]): number {
  const expected = new Set(expectedSymbols.map((symbol) => symbol.toLowerCase()));
  const index = nodes.findIndex((node) => expected.has(node.name.toLowerCase()));
