...
export class BunSqliteStorageAdapter implements StorageAdapter {
  ...
  transaction<T>(fn: (...a: unknown[]) => T): (...a: unknown[]) => T {
    return this.db.transaction(fn as (...a: unknown[]) => T) as (...a: unknown[]) => T;
  }
...
export function openSqliteStorage(path: string, options?: BunSqliteStorageOptions): StorageAdapter {
