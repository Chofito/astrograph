...
export async function openProject(rootPath: string, opts: OpenProjectOptions = {}): Promise<Astrograph> {
  const root = normalizePath(rootPath);
  const astrographDir = `${root}/.astrograph`;
  await mkdir(astrographDir, { recursive: true });

  const storage = new BunSqliteStorageAdapter(opts.dbPath ?? `${astrographDir}/graph.db`);
