...
export class QueryBuilder {
	...
	getNodesByFile(filePath: string): Node[] {
       ...
       ORDER BY file_path ASC, start_line ASC, kind ASC, qualified_name ASC`,
			...
			.all(filePath)
			.map((row) => mapNode(row as NodeRow));
	}
	...
	deleteByFile(filePath: string): {
		deletedNodes: number;
		deletedFiles: number;
	} {
		const remove = this.db.transaction(() => {
			const deletedNodes = this.db
				.prepare("DELETE FROM nodes WHERE file_path = ?")
				.run(filePath).changes;
			const deletedFiles = this.db
	...
	getFile(path: string): FileRecord | undefined {
		const row = this.db
			...
			.get(path) as FileRow | null | undefined;
	...
	deleteFile(path: string): number {
		return this.db.prepare("DELETE FROM files WHERE path = ?").run(path)
			.changes;
	}
	...
	getCoverage(paths?: string[]): Coverage {
		...
		const rows =
			paths === undefined
				? this.db
						.prepare(
							"SELECT state, COUNT(*) AS count FROM files GROUP BY state ORDER BY state ASC",
						)
						.all()
	...
	private selectCoverageForPaths(
		paths: string[],
	...
	private countBy(
		table: "nodes" | "edges" | "files",
		column: "kind" | "language",
