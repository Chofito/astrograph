...
describe('core storage', () => {
  test('round-trips files, nodes, and edges through SQLite', () => {
    ...
    const file = makeFile({ path: 'src/auth.ts', state: 'resolved', nodeCount: 2 });
    const source = makeNode({ id: 'node:auth-file', kind: 'file', name: 'auth.ts' });
    ...
    const edge = query.upsertEdge(makeEdge({
      source: source.id,
      target: target.id,
      targetName: 'login',
  ...
  test('cascades edges when deleting nodes for a file', () => {
    ...
    query.upsertFile(makeFile({ path: 'src/caller.ts', state: 'resolved', nodeCount: 1 }));
    query.upsertFile(makeFile({ path: 'src/callee.ts', state: 'resolved', nodeCount: 1 }));
  ...
  test('FTS search returns expected ids and coverage counts are correct', () => {
    ...
    query.upsertFile(makeFile({ path: 'src/alpha.ts', state: 'resolved', nodeCount: 1 }));
    query.upsertFile(makeFile({ path: 'src/beta.ts', state: 'parsed', nodeCount: 1 }));
    query.upsertFile(makeFile({ path: 'src/gamma.ts', state: 'pending', nodeCount: 0 }));
    query.upsertNode(alpha);
    query.upsertNode(beta);

    expect(query.search({ query: 'AlphaService' }).map((result) => result.node.id)).toEqual(['node:alpha']);
    expect(query.search({ query: 'BetaService' })).toEqual([]);
    expect(query.search({ query: 'BetaService', includeGenerated: true }).map((result) => result.node.id)).toEqual(['node:beta']);
  ...
  test('sync-support lookups are deterministic', () => {
    ...
    const target = makeNode({ id: 'node:target', name: 'target', qualifiedName: 'src/b.ts::target' });

    query.upsertNode(source);
    query.upsertNode(target);
    query.upsertEdge(makeEdge({ source: source.id, target: target.id, kind: 'calls', line: 10 }));
    query.upsertEdge(makeEdge({
      source: source.id,
      target: null,
    ...
    expect(query.getEdgesByTarget(target.id).map((edge) => edge.source)).toEqual([source.id]);
    expect(
      query.getEdgesByResolutionStateAndTargetName('unresolved', 'LaterSymbol')
