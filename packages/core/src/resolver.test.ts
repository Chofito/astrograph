...
describe("Pass B: edge resolution", () => {
    ...
    `,
		...
		try {
			...
			const importsToGreet = importEdges.filter(
				(e) => e.target === greetNode!.id,
			);
    ...
    `,
		...
		try {
			...
			const extendsBase = extendsEdges.filter(
				(e) => e.target === baseServiceNode!.id,
			);
			expect(extendsBase).toHaveLength(1);
			expect(extendsBase[0]!.resolutionState).toBe("resolved");

			const implementsLoggable = implementsEdges.filter(
				(e) => e.target === loggableNode!.id,
			);
    ...
    `,
		...
		try {
			...
			const callsToHelper = callEdges.filter(
				(e) => e.target === helperNode!.id,
			);
    ...
    `,
		...
		try {
			...
			const externalImports = importEdges.filter(
				(e) => e.resolutionState === "external",
			);
			...
			if (externalImportTarget === null)
				throw new Error("Expected external import target");

			const externalCalls = callEdges.filter(
				(e) => e.resolutionState === "external",
			);
			...
			expect(projectNodeFiles.has("node_modules/fake-lib/index.d.ts")).toBe(
				false,
			);
		} finally {
			indexer.close();
    ...
    `,
		...
		try {
			...
			const userNode = indexer.queries
				.getNodesByFile("src/types.ts")
				.find((n) => n.name === "User");
			expect(userNode).toBeDefined();

			const edges = indexer.queries.getAllEdges();
			const typeOfUser = edges.filter(
				(e) => e.kind === "type_of" && e.target === userNode!.id,
			);
			expect(typeOfUser).toHaveLength(2);
			expect(typeOfUser.map((e) => e.resolutionState)).toEqual([
				"resolved",
				"resolved",
			]);

			const returnsUser = edges.filter(
				(e) => e.kind === "returns" && e.target === userNode!.id,
			);
    ...
    `,
		...
		try {
			...
			const baseRun = nodes.find(
				(n) => n.name === "run" && n.qualifiedName.endsWith("BaseService.run"),
			);
			const userRun = nodes.find(
				(n) => n.name === "run" && n.qualifiedName.endsWith("UserService.run"),
			);
			expect(baseRun).toBeDefined();
			expect(userRun).toBeDefined();

			const overrides = indexer.queries
				.getAllEdges()
				.filter(
					(e) =>
						e.kind === "overrides" &&
    ...
    `,
		...
		try {
			...
			const decorates = indexer.queries
				.getAllEdges()
				.filter((e) => e.kind === "decorates");
			const decoratesInjectable = decorates.filter(
				(e) => e.target === injectable!.id,
			);
			...
			expect(decorates.map((e) => e.resolutionState)).toEqual([
				"resolved",
				"resolved",
			]);
    ...
    `,
		...
		try {
			...
			const references = indexer.queries
				.getAllEdges()
				.filter(
					(e) =>
						e.kind === "references" &&
    ...
    `,
		...
		try {
			...
			const greetBefore = firstIndexer.queries
				.getNodesByFile("src/utils.ts")
				.find((n) => n.name === "greet");
			expect(greetBefore).toBeDefined();
    ...
    `,
		...
		try {
			...
			const importsToGreet = allEdges.filter(
				(e) => e.kind === "imports" && e.target === greetBeforeId,
			);
			expect(importsToGreet).toHaveLength(1);
			expect(importsToGreet[0]!.resolutionState).toBe("resolved");

			const callsToGreet = allEdges.filter(
				(e) => e.kind === "calls" && e.target === greetBeforeId,
			);
			expect(callsToGreet).toHaveLength(2);
			expect(callsToGreet.map((e) => e.resolutionState)).toEqual([
				"resolved",
				"resolved",
			]);
    ...
    `,
		...
		try {
			...
			const useAccountNode = accountNodes.find(
				(node) => node.name === "useAccount",
			);
			...
			const calls = indexer.queries
				.getAllEdges()
				.filter((edge) => edge.kind === "calls");
			expect(
				calls.some(
					(edge) =>
						edge.source === screenNode!.id &&
						edge.target === useAccountNode!.id,
				),
			).toBe(true);
			expect(
				calls.some(
					(edge) =>
						edge.source === screenNode!.id && edge.target === helperNode!.id,
				),
			).toBe(true);
			expect(
				calls.some(
					(edge) =>
						edge.source === runNode!.id && edge.target === helperNode!.id,
				),
			).toBe(true);
			expect(
				calls.some(
					(edge) =>
						edge.source === methodNode!.id && edge.target === helperNode!.id,
				),
			).toBe(true);
			expect(
				calls.some(
					(edge) =>
						edge.source === fileNode!.id && edge.target === useAccountNode!.id,
				),
			).toBe(false);
			...
			expect(callees.data.map((item) => item.callee.name)).toContain(
				"useAccount",
			);
    ...
    `,
		...
		try {
			...
			const allEdges = indexer.queries.getAllEdges();
			const resolvedEdges = allEdges.filter(
				(e) => e.resolutionState === "resolved",
			);
