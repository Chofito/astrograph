...
export function normalize(graph: GraphLike, options: NormalizeOptions = {}): NormalizedGraph {
  return {
    nodes: graph.nodes.map((node) => dropVolatileNodeFields(node, options)).sort(compareNodes),
