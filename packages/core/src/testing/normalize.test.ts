...
describe('normalize', () => {
  test('drops volatile fields, strips absolute roots, and sorts deterministically', () => {
    ...
    const edges: Edge[] = [
      makeEdge({ id: 2, source: 'node:b', target: 'node:a', kind: 'references', line: 2 }),
      makeEdge({ id: 1, source: 'node:a', target: 'node:b', kind: 'calls', line: 1 }),
    ];

    const graph = normalize({ nodes, edges }, { rootPath: '/repo' });

    expect(graph.nodes.map((node) => node.filePath)).toEqual(['src/a.ts', 'src/b.ts']);
    expect(graph.nodes.map((node) => 'updatedAt' in node)).toEqual([false, false]);
    expect(graph.edges.map((edge) => edge.source)).toEqual(['node:a', 'node:b']);
