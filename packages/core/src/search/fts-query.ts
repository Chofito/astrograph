...
export function toFtsMatchQuery(raw: string): string {
  ...
  const tokens = contentTokens.length > 0 ? contentTokens : rawTokens.filter((token) => token.length > 0);
  const expandedTokens = tokens.flatMap(expandIdentifierToken);

  return uniqueSorted(expandedTokens).map((token) => `"${token}"*`).join(' OR ');
}
...
function expandIdentifierToken(token: string): string[] {
  const lower = token.includes('.') ? token.replaceAll('.', '').toLowerCase() : token.toLowerCase();
  const parts = splitIdentifier(token)
    .map((part) => part.toLowerCase())
    .filter((part) => part.length >= 3 && !STOPWORDS.has(part) && !FTS_BOOLEAN_OPERATORS.has(part));
  return [lower, ...parts];
}
