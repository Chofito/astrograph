...
export class TsExtractor implements Extractor {
	...
	loadProject(opts: LoadProjectOptions): void {
		...
		const projectFiles =
			opts.fileNames !== undefined
				? indexedFiles
				: this.program
						.getRootFileNames()
						.map((fileName) => this.toRelative(fileName));
	...
	extractNodes(
		...
		try {
			...
			nodes.push(
				this.makeFileNode(sourceFile, filePath, lang, generated, test),
			);
	...
	private visitNode(
		...
		if (ts.isInterfaceDeclaration(node)) {
			this.handleInterfaceDeclaration(
				node,
				sourceFile,
				source,
				lang,
		...
		if (ts.isTypeAliasDeclaration(node)) {
			this.handleTypeAliasDeclaration(
				node,
				sourceFile,
				source,
				lang,
	...
	private handleFunctionDeclaration(
		...
		const kind: NodeKind = isComponent(identity.name, node, sourceFile, lang)
			? "component"
			: "function";
		...
		for (const member of classNode.members) {
			if (
				...
				const existing = nodes.find(
					(n) =>
						n.qualifiedName === identity.qualifiedName && n.kind === "property",
				);
				if (!existing) {
					nodes.push(
						this.buildNode({
	...
	private handleInterfaceDeclaration(
		...
		for (const member of node.members) {
			if (ts.isMethodSignature(member)) {
				const memberIdentity = computeNodeIdentity(
					member,
					sourceFile,
					this.project,
					this.hasher,
	...
	private handleVariableStatement(
		...
		for (const decl of stmt.declarationList.declarations) {
			...
			if (init && (ts.isArrowFunction(init) || ts.isFunctionExpression(init))) {
				kind = isComponent(identity.name, init, sourceFile, lang, decl)
					? "component"
					: "function";
			}
...
function normalizeIndexedFiles(
...
function hasReactFCTyping(node: ts.Node): boolean {
	const typeNode = (node as ts.FunctionDeclaration | ts.VariableDeclaration)
		.type;
