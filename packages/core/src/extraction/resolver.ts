...
import type { Edge, ExtractionError, Hasher, Node, Confidence, Provenance } from '../types';
...
export function resolveEdgesForFile(opts: ResolverOptions): ResolverResult {
  ...
  function ensureExternalNode(sym: ts.Symbol, declFile: ts.SourceFile): string {
    ...
    const identity = computeNodeIdentity(decl, declFile, project, hasher, relPath);
  ...
  function makeExternalFallbackId(sym: ts.Symbol, declFile: ts.SourceFile): string {
    const relPath = toRelative(declFile.fileName);
  ...
  function resolveSymbol(
    ...
    if (resolved.flags & ts.SymbolFlags.Alias) {
      try {
        ...
        return { target: null, targetName: name, resolutionState: 'unresolved', confidence: 'low' };
      }
    }
    ...
    return { target, targetName: name, resolutionState: 'external', confidence: 'high' };
  }
  ...
  function getEnclosingDeclaration(node: ts.Node): ts.Node | undefined {
    ...
    while (current) {
      if (ts.isFunctionDeclaration(current) || ts.isMethodDeclaration(current) ||
          ...
          ts.isModuleDeclaration(current) || ts.isTypeAliasDeclaration(current) ||
  ...
  function findNodeIdInFile(astNode: ts.Node, targetFilePath: string, targetSourceFile: ts.SourceFile): string | undefined {
    const nodes = getNodesForFile(targetFilePath);
    if (nodes.length === 0) return undefined;

    const rangeNode = rangeNodeForDeclaration(astNode);
    const startLine = targetSourceFile.getLineAndCharacterOfPosition(rangeNode.getStart(targetSourceFile)).line + 1;
    const startColumn = targetSourceFile.getLineAndCharacterOfPosition(rangeNode.getStart(targetSourceFile)).character;
    const kind = kindFromDeclaration(astNode, targetSourceFile);
    const name = nameFromDeclaration(astNode, targetSourceFile);

    for (const node of nodes) {
      if (isCompatibleKind(kind, node.kind) &&
          node.name === name &&
  ...
  function symbolLocationForExpression(expr: ts.Expression): ts.Node {
    ...
    if (ts.isElementAccessExpression(expr) && expr.argumentExpression !== undefined) return expr.argumentExpression;
    return expr;
  }
  ...
  function emitResolvedEdge(input: {
    ...
    const pos = sourceFile.getLineAndCharacterOfPosition(input.location.getStart(sourceFile));
    ...
    if (resolved === undefined) {
      emitEdge({
        source: input.source,
        target: null,
        targetName: input.fallbackTargetName ?? input.location.getText(sourceFile),
        kind: input.kind,
  ...
  function emitContainsEdges(): void {
    ...
    function visitForContains(node: ts.Node, parentId: string | undefined): void {
      const declKinds = [
      ...
      if (ts.isMethodDeclaration(node) || ts.isConstructorDeclaration(node) ||
          ...
          ts.isMethodSignature(node) || ts.isPropertySignature(node) || ts.isEnumMember(node)) {
        const nodeId = getSourceIdForNode(node);
        if (parentId && nodeId) {
          const pos = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
          emitEdge({
    ...
    ts.forEachChild(sourceFile, (node) => {
      ...
      if (clause.name) {
        ...
        if (sym) {
          ...
          const pos = sourceFile.getLineAndCharacterOfPosition(clause.name.getStart(sourceFile));
          emitEdge({
            source: sourceId,
            target: resolved.target,
            targetName: resolved.targetName,
      ...
      if (clause.namedBindings) {
        if (ts.isNamespaceImport(clause.namedBindings)) {
          ...
          if (sym) {
            ...
            const pos = sourceFile.getLineAndCharacterOfPosition(clause.namedBindings.name.getStart(sourceFile));
            emitEdge({
              source: sourceId,
              target: resolved.target,
              targetName: resolved.targetName,
          ...
          for (const spec of clause.namedBindings.elements) {
            ...
            if (sym) {
              ...
              const pos = sourceFile.getLineAndCharacterOfPosition(spec.name.getStart(sourceFile));
              emitEdge({
                source: sourceId,
                target: resolved.target,
                targetName: resolved.targetName,
  ...
  function emitTypeAndReturnEdges(): void {
    function visit(node: ts.Node): void {
      if (ts.isVariableDeclaration(node) && node.type) {
        ...
        const sourceId = getSourceId(node);
        if (sourceId) emitTypeEdgesFromTypeNode(sourceId, node.type, 'type_of');
      } else if ((ts.isPropertyDeclaration(node) || ts.isPropertySignature(node)) && node.type) {
        const sourceId = getSourceIdForNode(node) ?? getSourceId(node);
        if (sourceId) emitTypeEdgesFromTypeNode(sourceId, node.type, 'type_of');
      }
  ...
  function emitTypeEdgesFromTypeNode(sourceId: string, typeNode: ts.TypeNode, kind: 'type_of' | 'returns'): void {
  ...
  function collectTypeReferenceLocations(typeNode: ts.TypeNode): ts.Node[] {
    ...
    const visitType = (node: ts.TypeNode): void => {
      ...
      if (ts.isTypeReferenceNode(node)) {
        ...
        if (!isBuiltinTypeReference(location.getText(sourceFile))) add(location);
    ...
    visitType(typeNode);
    return locations.sort((a, b) => a.getStart(sourceFile) - b.getStart(sourceFile));
  }
    ...
    function visit(node: ts.Node): void {
      if (ts.isClassDeclaration(node)) {
        for (const member of node.members) {
          ...
          const pos = sourceFile.getLineAndCharacterOfPosition(location.getStart(sourceFile));
          emitEdge({
            source: sourceId,
            target: resolved.target,
            targetName: resolved.targetName,
  ...
  function findOverriddenMethodSymbol(classDecl: ts.ClassDeclaration, method: ts.MethodDeclaration): ts.Symbol | undefined {
    ...
    const classType = classSymbol ? checker.getDeclaredTypeOfSymbol(classSymbol) : checker.getTypeAtLocation(classDecl);
  ...
  function findMethodInBaseTypes(methodName: string, baseTypes: readonly ts.BaseType[], seen: Set<ts.Type>): ts.Symbol | undefined {
    for (const baseType of baseTypes) {
      ...
      const nextBaseTypes = checker.getBaseTypes(baseType as ts.InterfaceType) ?? [];
  ...
  function decoratorSymbolLocation(decorator: ts.Decorator): ts.Node {
    ...
    if (ts.isCallExpression(expression)) return symbolLocationForExpression(expression.expression);
  ...
  function emitReferenceForLocation(location: ts.Node | undefined): void {
    ...
    if (resolved.target === null && resolved.resolutionState === 'unresolved') return;
  ...
  function jsxTagLocation(tagName: ts.JsxTagNameExpression): ts.Node | undefined {
  ...
  function shouldSkipReferenceIdentifier(node: ts.Identifier): boolean {
    ...
    if (hasAncestor(node, (parent) => ts.isImportDeclaration(parent) || ts.isImportEqualsDeclaration(parent))) return true;
    if (hasAncestor(node, (parent) => ts.isExportDeclaration(parent) || ts.isExportAssignment(parent))) return true;
  ...
  function hasAncestor(node: ts.Node, predicate: (node: ts.Node) => boolean): boolean {
    let current: ts.Node | undefined = node.parent;
...
function hasTsIgnoreDirective(node: ts.Node, sourceFile: ts.SourceFile): boolean {
  const ranges = ts.getLeadingCommentRanges(sourceFile.text, node.getFullStart());
  if (!ranges) return false;
  for (const range of ranges) {
    const text = sourceFile.text.slice(range.pos, range.end);
    if (text.includes('@ts-ignore') || text.includes('@ts-expect-error')) return true;
  }
  return false;
}
