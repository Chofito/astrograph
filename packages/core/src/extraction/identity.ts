...
export function kindFromDeclaration(node: ts.Node, sourceFile: ts.SourceFile): NodeKind {
  ...
  if (ts.isMethodDeclaration(node) || ts.isConstructorDeclaration(node)) return 'method';
...
export function nameFromDeclaration(node: ts.Node, sourceFile: ts.SourceFile): string {
  ...
  if (ts.isMethodDeclaration(node)) return node.name?.getText(sourceFile) ?? '<anonymous>';
  if (ts.isConstructorDeclaration(node)) return 'constructor';
  if (ts.isMethodSignature(node)) return node.name?.getText(sourceFile) ?? '<anonymous>';
  if (ts.isPropertyDeclaration(node) || ts.isPropertySignature(node)) return node.name?.getText(sourceFile) ?? '<anonymous>';
  if (ts.isGetAccessor(node) || ts.isSetAccessor(node)) return node.name?.getText(sourceFile) ?? '<anonymous>';
...
function findOverloadSiblings(node: ts.Node, sourceFile: ts.SourceFile): ts.Node[] {
  if (ts.isFunctionDeclaration(node) && node.name) {
    const name = node.name.text;
    const container = node.parent;
    const statements = ts.isSourceFile(container) ? container.statements :
      ts.isModuleBlock(container) ? container.statements :
      undefined;
    if (!statements) return [node];
    return statements.filter((s): s is ts.FunctionDeclaration =>
  ...
  if (ts.isMethodDeclaration(node)) {
    ...
    if ((ts.isClassDeclaration(parent) || ts.isClassExpression(parent)) && name) {
      return parent.members.filter((m): m is ts.MethodDeclaration =>
  ...
  if (ts.isMethodSignature(node)) {
    ...
    if (ts.isInterfaceDeclaration(parent) && name) {
      return parent.members.filter((m): m is ts.MethodSignature =>
...
function computeOverloadLocator(node: ts.Node, sourceFile: ts.SourceFile, hasher: Hasher): string | undefined {
  const siblings = findOverloadSiblings(node, sourceFile);
  if (siblings.length <= 1) return undefined;

  const sorted = [...siblings].sort((a, b) => a.getStart(sourceFile) - b.getStart(sourceFile));
...
export function extractDocstring(node: ts.Node, sourceFile: ts.SourceFile, source: string): string | undefined {
...
export function extractDecorators(node: ts.Node, sourceFile: ts.SourceFile): string[] {
...
export function getVisibility(node: ts.Node): 'public' | 'private' | 'protected' | undefined {
