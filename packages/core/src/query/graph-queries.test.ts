...
describe("GraphQueries", () => {
	test("callers and callees return exact node refs and call-site lines", async () => {
		...
		try {
			...
			expect(
				callers.data.map((item) => ({
					caller: item.caller,
					callSite: callSiteFields(item.callSite),
				})),
			).toEqual([
				{
	...
	test("callers and callees hide external nodes by default and include them on opt-in", async () => {
		...
		try {
			const run = mustNode(graph, "run");
			const helper = mustNode(graph, "helper");
			const externalFn = graph.queries
				.getAllNodes()
				.find((node) => node.name === "externalFn" && node.isExternal);
			expect(externalFn).toBeDefined();

			const defaultCallees = await graph.callees({ symbol: "run" });
			expect(defaultCallees.data.map((item) => item.callee.name)).toEqual([
				"helper",
			]);

			const withExternalCallees = await graph.callees({
				symbol: "run",
				includeExternal: true,
			});
			expect(withExternalCallees.data.map((item) => item.callee.name)).toEqual([
				"externalFn",
				"helper",
			]);
			...
			expect(defaultCallers.data.map((item) => item.caller.name)).toEqual([
				run.name,
			]);

			const withExternalCallers = await graph.callers({
				symbol: "helper",
				includeExternal: true,
			});
			expect(withExternalCallers.data.map((item) => item.caller.name)).toEqual([
				"externalFn",
				run.name,
			]);
		} finally {
	...
	test("impact is bounded by depth and deterministic", async () => {
		...
		try {
			...
			expect(
				depthOne.data.map((item) => [item.node.name, item.distance]),
			).toEqual([["middle", 1]]);
			expect(
				depthTwo.data.map((item) => [item.node.name, item.distance]),
			).toEqual([
				["middle", 1],
				["root", 2],
			]);
	...
	test("trace returns a hop chain with bodies and fallback code when no path exists", async () => {
		const graph = await indexFixtureProject();
		try {
			const trace = await graph.trace({
				from: "root",
				to: "leaf",
				maxDepth: 3,
			});
			expect(trace.data.found).toBe(true);
			expect(trace.data.hops.map((hop) => hop.node.name)).toEqual([
				"root",
				"middle",
			]);
			expect(trace.data.hops.map((hop) => hop.via.kind)).toEqual([
				"calls",
				"calls",
			]);
			expect(trace.data.hops[0]!.body.content).toBe(
				["export function root() {", "  return middle();", "}"].join("\n"),
			);
			expect(trace.data.destinationCallees).toEqual([]);

			const noPath = await graph.trace({
				from: "leaf",
				to: "root",
				maxDepth: 2,
			});
			...
			expect(
				noPath.data.endpoints?.map((endpoint) => endpoint.node.name),
			).toEqual(["leaf", "root", "middle"]);
			expect(
				noPath.data.endpoints?.map((endpoint) => endpoint.body.filePath),
			).toEqual(["src/chain.ts", "src/chain.ts", "src/chain.ts"]);
		} finally {
			graph.close();
		}
	});
	...
	test("context returns ranked entry points, neighborhood code, reasons, and stats", async () => {
		...
		try {
			...
			expect(
				result.data.entryPoints.some((node) => node.id === helper.id),
			).toBe(true);
			expect(
				result.data.subgraph.nodes.some((node) => node.id === run.id),
			).toBe(true);
			...
			expect(
				result.data.entryPoints.every(
					(node) => !node.filePath.includes("node_modules"),
				),
			).toBe(true);
			expect(
				result.data.subgraph.nodes.every(
					(node) => !node.filePath.includes("node_modules"),
				),
			).toBe(true);
			expect(result.data.stats).toEqual({
				...
				totalCodeChars: result.data.codeBlocks.reduce(
	...
	test("context respects token budgets for code blocks", async () => {
		...
		try {
			...
			expect(result.data.stats.codeBlockCount).toBe(
				result.data.codeBlocks.length,
			);
		} finally {
			graph.close();
		}
	...
	test("context ranks generated and test symbols below real implementations with the same name", async () => {
		...
		try {
			...
			const duplicateNodes = result.data.subgraph.nodes.filter(
				(node) => node.name === "duplicateName",
			);
			...
			expect(duplicatePaths.indexOf("src/ranked.ts")).toBeLessThan(
				duplicatePaths.indexOf("src/ranked.test.ts"),
			);
			expect(duplicatePaths.indexOf("src/ranked.ts")).toBeLessThan(
				duplicatePaths.indexOf("src/ranked.generated.ts"),
			);
		} finally {
			graph.close();
		}
	...
	test("getNode includeCode returns the verbatim node slice", async () => {
		...
		try {
			...
			expect(result.data.code?.content).toBe(
				["  run() {", "    return helper('Ada') + externalFn();", "  }"].join(
					"\n",
				),
			);
			expect(result.meta.partial).toBe(false);
	...
	test("explore groups code by file and maps relationships among included nodes", async () => {
		...
		try {
			...
			expect(result.data.files.map((file) => file.filePath)).toEqual([
				"src/a.ts",
				"src/b.ts",
			]);
			expect(
			...
			expect(
				result.data.files.map((file) =>
					file.blocks.map((block) => block.filePath),
				),
			).toEqual([["src/a.ts"], ["src/b.ts"]]);
			expect(
				result.data.relationshipMap.map((edge) => [
					edge.kind,
					edge.resolutionState,
				]),
			).toEqual([["calls", "resolved"]]);
		} finally {
			graph.close();
		}
	});
	...
	test("getFiles and getStats expose coverage and file metadata", async () => {
		...
		try {
			...
			expect(status.data.coverage).toEqual({
				total: indexedFiles.length,
				resolved: indexedFiles.length,
				parsed: 0,
				pending: 0,
			});
	...
	test("search is deterministic and wraps the coverage envelope", async () => {
		...
		try {
			...
			expect(first.meta.coverage).toEqual({
				total: graph.queries.getAllFiles().length,
				resolved: graph.queries.getAllFiles().length,
				parsed: 0,
				pending: 0,
			});
		} finally {
...
async function indexFixtureProject(): Promise<Astrograph> {
	...
	await writeProjectFile(
		...
		["export declare function externalFn(): string;", ""].join("\n"),
	);
	await writeProjectFile(
		root,
	...
	await writeProjectFile(
		root,
		"src/ranked.test.ts",
		["export function duplicateName() {", "  return 'test';", "}", ""].join(
			"\n",
		),
	);
...
function mustNode(
	graph: Astrograph,
	name: string,
): ReturnType<Astrograph["queries"]["getAllNodes"]>[number] {
