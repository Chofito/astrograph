...
export class GraphQueries {
	...
	async context(input: ContextInput): Promise<ToolResult<ContextOutput>> {
		...
		const projectEntries = entryResults
			.map((result) => {
				...
				return node === undefined || node.isExternal
					? undefined
					: { result, node };
			})
			.filter(
				(entry): entry is { result: SearchOutput[number]; node: Node } =>
					entry !== undefined,
			);
		const entryNodes = projectEntries.map((entry) => entry.node);
		const entryIds = new Set(entryNodes.map((node) => node.id));
		const ftsScores = normalizedFtsScores(
			projectEntries.map((entry) => entry.result),
		);
		...
		for (const entry of entryNodes) {
			for (const direction of ["outgoing", "incoming"] as const) {
				...
				for (const visit of visits) {
					...
					const reason = entryIds.has(visit.node.id)
						? "fts-match"
						: this.contextReason(visit, direction);
					const current = candidates.get(visit.node.id);
					if (
						current === undefined ||
						visit.distance < current.distance ||
						(visit.distance === current.distance &&
							compareStrings(reason, current.reason) < 0)
					) {
						candidates.set(visit.node.id, {
							node: visit.node,
		...
		const degreeScores = normalizedDegreeScores(
			this.queries,
			[...candidates.values()].map((candidate) => candidate.node),
		);
		...
		const codeBlocks =
			input.includeCode === false
				? []
				: await this.contextCodeBlocks(
						ranked.map((candidate) => candidate.node),
						input.tokenBudget,
					);
		const relatedFiles = filesForNodes(
			ranked.map((candidate) => candidate.node),
		);
		const totalCodeChars = codeBlocks.reduce(
	...
	async getNode(input: NodeInput): Promise<ToolResult<NodeOutput>> {
		...
		const calleesPreview = this.refsForEdgeNodes(calleesEdges, "target").slice(
			0,
			5,
		);
		const code =
			input.includeCode === true
				? await this.slicer.sliceNode(node)
				: undefined;
		...
		const data = sortCallerOutputs(
			edges
				.map((edge) => {
					...
					if (caller?.isExternal === true && input.includeExternal !== true)
						return undefined;
				...
				.filter(
					(result): result is CallersOutput[number] => result !== undefined,
				),
		).slice(0, limit);
		...
		const data = sortCalleeOutputs(
			edges
				.map((edge) => {
					...
					if (callee?.isExternal === true && input.includeExternal !== true)
						return undefined;
				...
				.filter(
					(result): result is CalleesOutput[number] => result !== undefined,
				),
		).slice(0, limit);
	...
	async impact(input: ImpactInput): Promise<ToolResult<ImpactOutput>> {
		...
		const data = visits
			...
			.filter(
				(visit) => input.includeExternal === true || !visit.node.isExternal,
			)
			.map((visit) => ({
		...
		if (path !== undefined) {
			const hops = await Promise.all(
				path.map(async (edge) => {
					...
					return {
						node: toNodeRef(source),
						via: toEdgeRef(edge),
						body: await this.slicer.sliceNode(source),
					};
				}),
			);
			...
			return {
				data: {
					found: true,
					hops: hops.filter(
						(hop): hop is TraceOutput["hops"][number] => hop !== undefined,
					),
					destinationCallees,
				},
				meta: this.meta({
	...
	async explore(input: ExploreInput): Promise<ToolResult<ExploreOutput>> {
		...
		const nodes = uniqueNodes(
			terms.flatMap((term) => resolveSymbol(this.queries, term).candidates),
		).filter((node) => !node.isExternal);
		const nodesByFile = groupNodesByFile(nodes, maxFiles);
		const files = await Promise.all(
			[...nodesByFile.entries()].map(async ([filePath, fileNodes]) => ({
				filePath,
				blocks: await Promise.all(
					fileNodes.map((node) => this.slicer.sliceNode(node)),
				),
			})),
		);
		const includedIds = new Set(
			[...nodesByFile.values()].flat().map((node) => node.id),
		);
		...
		const entries = this.queries
			...
			.filter(
				(file) =>
					...
					matchesPattern(file.path, input.pattern),
			)
			.filter(
				(file) =>
					input.maxDepth === undefined ||
					pathDepth(file.path) <= input.maxDepth,
			)
			.map(
				(file): FileEntry => ({
					filePath: file.path,
	...
	private relationshipsAmong(nodeIds: Set<string>): EdgeRef[] {
		const edges = [...nodeIds]
			.flatMap((id) => this.queries.getEdgesBySource(id))
	...
	private async contextCodeBlocks(
		nodes: Node[],
		...
		for (const node of nodes) {
			...
			if (tokenBudget !== undefined && usedTokens + tokens > tokenBudget)
				continue;
			blocks.push(block);
			usedTokens += tokens;
		}
	...
	private contextReason(
		visit: { node: Node; distance: number; path: Edge[] },
		...
		const source = this.queries.getNode(edge.source);
		const target =
			edge.target === null ? undefined : this.queries.getNode(edge.target);
		const sourceName = source?.name ?? edge.source;
		const targetName =
			target?.name ?? edge.targetName ?? edge.target ?? "unknown";
	...
	private traceFallbackNodes(from: Node, to: Node): Node[] {
		const siblings = this.queries
			.getNodesByFile(to.filePath)
			.filter(
				(node) => node.id !== to.id && node.kind !== "file" && !node.isExternal,
			)
			.sort(compareNodes)
			.slice(0, 5);
...
function normalizedDegreeScores(
	queries: QueryBuilder,
	nodes: Node[],
): Map<string, number> {
	const kinds = new Set<EdgeKind>(CONTEXT_EDGE_KINDS);
	...
	for (const node of nodes) {
		const outgoing = queries
			...
			.filter((edge) => kinds.has(edge.kind)).length;
		const incoming = queries
			.getEdgesByTarget(node.id)
			.filter((edge) => kinds.has(edge.kind)).length;
...
function compareContextCandidates(
	a: ScoredContextCandidate,
	b: ScoredContextCandidate,
): number {
	return (
		b.score - a.score ||
...
function edgeNotes(edges: Edge[]): string[] {
	const included = new Set(
		edges.map(
			(edge) =>
				edge.id ??
				`${edge.source}:${edge.kind}:${edge.target ?? edge.targetName ?? ""}`,
		),
	);
	const ambiguous = edges.filter(
	...
	const external = edges.filter(
		(edge) => edge.resolutionState === "external",
	).length;
	...
	if (included.size !== edges.length)
		notes.push("Duplicate edge rows collapsed in output");
	return notes;
}
...
function ambiguityNotes(edges: Edge[], names: string[]): string[] {
	...
	return duplicates > 0
		? [...notes, `${duplicates} duplicate symbol name(s) in result candidates`]
		: notes;
}
...
function filesForNodes(nodes: Node[]): string[] {
	return [
		...new Set(
			nodes.filter((node) => !node.isExternal).map((node) => node.filePath),
		),
	].sort(compareStrings);
}
...
function sortCallerOutputs(outputs: CallersOutput): CallersOutput {
	return [...outputs].sort(
		(a, b) =>
			compareNodeRefs(a.caller, b.caller) ||
			compareEdgeRefs(a.callSite, b.callSite),
	);
}
...
function sortCalleeOutputs(outputs: CalleesOutput): CalleesOutput {
	return [...outputs].sort(
		(a, b) =>
			compareNodeRefs(a.callee, b.callee) ||
			compareEdgeRefs(a.callSite, b.callSite),
	);
}
...
function compareImpactOutputs(
	a: ImpactOutput[number],
	b: ImpactOutput[number],
): number {
