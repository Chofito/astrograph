...
describe('indexAll onProgress hook', () => {
  ...
  test('IndexProgress supports all phases', () => {
    const phases: IndexProgress['phase'][] = ['scan', 'parse', 'resolve', 'done'];
  ...
  test('onProgress callback receives monotonic progress', async () => {
    ...
    const mockIndexAll = async (opts?: { onProgress?: (e: IndexProgress) => void }) => {
      opts?.onProgress?.({ phase: 'scan', current: 10, total: 10 });
      for (let i = 1; i <= 10; i++) {
        opts?.onProgress?.({ phase: 'parse', current: i, total: 10, file: `file${i}.ts` });
      }
      for (let i = 1; i <= 10; i++) {
        opts?.onProgress?.({ phase: 'resolve', current: i, total: 10, file: `file${i}.ts` });
      }
    ...
    for (let i = 1; i < parseEvents.length; i++) {
      expect(parseEvents[i]!.current).toBeGreaterThan(parseEvents[i - 1]!.current);
      expect(parseEvents[i]!.current).toBeLessThanOrEqual(parseEvents[i]!.total);
    }
  });
});
