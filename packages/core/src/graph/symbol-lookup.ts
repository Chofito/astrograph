...
export function resolveSymbol(queries: QueryBuilder, symbol: string): SymbolLookupResult {
  ...
  const ftsMatches = queries.search({ query: symbol, limit: 50, includeGenerated: true })
...
function compareNodes(a: Node, b: Node): number {
  return Number(a.isGenerated) - Number(b.isGenerated)
