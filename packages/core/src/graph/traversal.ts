...
export function traverseGraph(queries: QueryBuilder, input: TraverseInput): TraversalVisit[] {
  ...
  while (queue.length > 0 && visits.length < input.limit) {
    ...
    const edges = input.direction === 'outgoing'
      ? queries.getEdgesBySource(current.node.id)
      : queries.getEdgesByTarget(current.node.id);

    for (const edge of sortEdges(edges).filter((e) => edgeKinds === undefined || edgeKinds.has(e.kind))) {
...
export function findPath(
  ...
  const edgeKinds = input.edgeKinds === undefined ? undefined : new Set(input.edgeKinds);
  const seen = new Set<string>([start.id]);
  const queue: { nodeId: string; distance: number; path: Edge[] }[] = [{ nodeId: start.id, distance: 0, path: [] }];
  let visited = 0;
  ...
  while (queue.length > 0 && visited < limit) {
    ...
    for (const edge of sortEdges(edges).filter((e) => edgeKinds === undefined || edgeKinds.has(e.kind))) {
      ...
      seen.add(nextId);
      queue.push({ nodeId: nextId, distance: current.distance + 1, path: [...current.path, edge] });
    }
