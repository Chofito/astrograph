...
export class FreshnessManager {
	...
	private readonly onSyncError:
		| ((events: WatchEvent[], error: unknown) => void)
		| undefined;
	...
	start(): boolean {
		...
		try {
			this.watchHandle = this.watcher.watch([this.root], (event) =>
	...
	decorateResult<T>(result: ToolResult<T>): ToolResult<T> {
		...
		return {
			...
			meta: {
				...result.meta,
				partial:
					result.meta.partial || pendingFiles.length > 0 || this.unavailable,
				pendingFiles: uniqueSorted([
					...(result.meta.pendingFiles ?? []),
					...pendingFiles,
				]),
	...
	recordEvent(event: WatchEvent): void {
		...
		this.pendingEvents.set(
			relPath,
			mergeEvent(this.pendingEvents.get(relPath), normalizedEvent),
		);
	...
	private drainPending(): WatchEvent[] {
		const events = [...this.pendingEvents.values()].sort((a, b) =>
