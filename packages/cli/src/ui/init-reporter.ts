...
export function createInitReporter(
...
class FancyInitReporter implements InitReporter {
	...
	private lines(): string[] {
		...
		const file =
			this.current.file === undefined
				? this.ttyStyle.subtle("discovering project files")
				: this.ttyStyle.subtle(
						truncateMiddle(
							this.current.file,
							Math.max(24, (this.stream.columns ?? 80) - 18),
						),
					);
...
function phaseLabel(phase: IndexProgress["phase"]): {
	icon: string;
	label: string;
} {
	switch (phase) {
		case "scan":
			return { icon: symbols.bullet, label: "Scanning" };
		case "parse":
			return { icon: "◐", label: "Parsing" };
		case "resolve":
			return { icon: "◒", label: "Resolving" };
		case "done":
			return { icon: symbols.success, label: "Complete" };
	}
}
