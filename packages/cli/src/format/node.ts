...
export function formatNode(result: ToolResult<NodeOutput>): string {
  ...
  if (result.data.node.signature !== undefined) lines.push(`signature ${result.data.node.signature}`);
  if (result.data.callersPreview.length > 0) lines.push(`callers ${names(result.data.callersPreview)}`);
  if (result.data.calleesPreview.length > 0) lines.push(`callees ${names(result.data.calleesPreview)}`);
