...
export function formatStatus(result: ToolResult<StatusOutput>, daemon?: DaemonStatus): string {
  const lines = [
  ...
  if (daemon) {
    if (daemon.running && daemon.pid !== undefined) {
      const since = daemon.startedAt ? formatSince(daemon.startedAt) : 'unknown';
      lines.push(`${symbols.bullet} daemon     running (pid ${style.num(daemon.pid)}, since ${style.dim(since)})`);
    } else {
