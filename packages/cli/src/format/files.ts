...
export function formatFiles(
	result: ToolResult<FilesOutput>,
	includeMetadata = true,
): string {
	const rows =
		result.data.format === "grouped"
...
function flatRows(data: FilesOutput, includeMetadata: boolean): string[] {
	return data.entries.map((entry) =>
		row(entry.filePath, entry, includeMetadata),
	);
}
...
function groupedRows(data: FilesOutput, includeMetadata: boolean): string[] {
	...
	return [...groups.entries()]
		...
		.flatMap(([language, entries]) => [
			language,
			...entries.map(
				(entry) => `  ${row(entry.filePath, entry, includeMetadata)}`,
			),
		]);
}
...
function row(
	label: string,
	entry: FilesOutput["entries"][number],
