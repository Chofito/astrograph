...
export async function uninstallAgentGuide(link: AgentGuideLink): Promise<AgentGuideResult> {
  ...
  if (readInstalledGuide(link) !== embeddedGuideText) {
    return { path: link.path, action: 'skipped', reason: 'existing non-symlink file' };
  }
...
async function installExternalAgentGuide(
  ...
  if (!existsSync(source)) {
    return { path: link.path, action: 'skipped', reason: `missing guide source: ${source}` };
  }
...
async function installEmbeddedAgentGuide(link: AgentGuideLink): Promise<AgentGuideResult> {
  ...
  await writeEmbeddedGuide(link);
  return { path: link.path, action: current === embeddedGuideText ? 'unchanged' : 'installed' };
}
