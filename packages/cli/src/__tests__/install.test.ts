...
async function makeTmpHome(): Promise<{
	root: string;
	cwd: string;
	homeDir: string;
}> {
...
async function expectGuideInstalled(
	path: string,
...
describe("claude target", () => {
	...
	test("install into local path writes to .mcp.json", async () => {
		...
		try {
			const result = await runInstallCore(
				["--target", "claude", "--location", "local", "--yes"],
				ctx(cwd),
				homeDir,
			);
			expect(result.exitCode).toBe(0);
	...
	test("install preserves existing mcpServers and other keys", async () => {
		...
		try {
			...
			expect(parsed.mcpServers.otherServer).toEqual({
				type: "stdio",
				command: "other",
				args: [],
			});
	...
	test("install twice is idempotent", async () => {
		...
		try {
			await runInstallCore(["--target", "claude", "--yes"], ctx(cwd), homeDir);
			const result2 = await runInstallCore(
				["--target", "claude", "--yes"],
				ctx(cwd),
				homeDir,
			);
	...
	test("uninstall removes only astrograph and leaves the rest", async () => {
		...
		try {
			...
			const result = await runUninstallCore(
				["--target", "claude", "--yes"],
				ctx(cwd),
				homeDir,
			);
...
describe("cursor target", () => {
	...
	test("local install writes the Cursor rule", async () => {
		...
		try {
			const result = await runInstallCore(
				["--target", "cursor", "--location", "local", "--yes"],
				ctx(cwd),
				homeDir,
			);
			expect(result.exitCode).toBe(0);
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({
					mcpServers: {
						other: { command: "x" },
						astrograph: { command: "astrograph" },
					},
				}),
				"utf8",
			);

			await runUninstallCore(
				["--target", "cursor", "--yes"],
				ctx(cwd),
				homeDir,
			);
	...
	test("uninstall when absent is a no-op", async () => {
		const { root, cwd, homeDir } = await makeTmpHome();
		try {
			const r = await runUninstallCore(
				["--target", "cursor", "--yes"],
				ctx(cwd),
				homeDir,
			);
...
describe("codex target", () => {
	...
	test("install writes the Codex skill", async () => {
		...
		try {
			...
			expect(result.exitCode).toBe(0);
			await expectGuideInstalled(
				join(homeDir, ".codex", "skills", "astrograph"),
				"directory",
			);
		} finally {
			await rm(root, { recursive: true, force: true });
	...
	test("--location local warns and skips codex", async () => {
		...
		try {
			const result = await runInstallCore(
				["--target", "codex", "--location", "local", "--yes"],
				ctx(cwd),
				homeDir,
			);
			expect(result.exitCode).toBe(0);
...
describe("opencode target", () => {
	...
	test("install into local path writes to ./opencode.jsonc", () =>
		withXdgTmp(async ({ cwd, homeDir }) => {
			await runInstallCore(
				["--target", "opencode", "--location", "local", "--yes"],
				ctx(cwd),
				homeDir,
			);
	...
	test("install with existing JSONC that has comments preserves comments", () =>
		withXdgTmp(async ({ cwd, homeDir, filePath }) => {
			...
			expect(parsed.mcp.astrograph.command).toEqual([
				"astrograph",
				"serve",
				"--mcp",
			]);
...
describe("cross-cutting", () => {
	test("--command overrides the binary in all targets", async () => {
		...
		try {
			...
			expect(parsed.mcpServers.astrograph.command).toBe(
				"/usr/local/bin/astrograph",
			);
