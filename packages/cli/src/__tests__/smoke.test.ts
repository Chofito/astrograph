...
describe("CLI command smoke", () => {
	...
	test("callees hides external symbols by default and includes them with the flag", async () => {
		...
		try {
			...
			await writeFile(
				`${root}/node_modules/fake-lib/index.d.ts`,
				["export declare function externalFn(): string;", ""].join("\n"),
				"utf8",
			);
			await writeFile(
				`${root}/node_modules/fake-lib/package.json`,
				[
			...
			const defaultCallees = await runCallees(["run", "-p", root], {
				cwd: root,
			});
			...
			const withExternal = await runCallees(
				["run", "-p", root, "--include-external"],
				{ cwd: root },
			);
			expect(withExternal.exitCode).toBe(0);
			expect(withExternal.stdout).toContain("function helper  src/a.ts:3");
			expect(withExternal.stdout).toContain(
				"function externalFn  node_modules/fake-lib/index.d.ts:0",
			);
		} finally {
			await rm(root, { recursive: true, force: true });
		}
