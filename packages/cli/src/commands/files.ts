...
import { booleanValue, numberValue, parseCommandArgs, readFlags, readOptions, stringValue } from './parse';
import { openGraphForRead } from './shared';

export async function runFiles(args: string[], ctx: CliContext): Promise<CliRunResult> {
  ...
  const format = (stringValue(parsed.values, 'format') ?? 'tree') as FilesInput['format'];
