...
export async function runDaemon(
	...
	const root = requireProjectRoot(
		resolveProjectPath(ctx.cwd, stringValue(parsed.values, "path")),
	);
	...
	try {
		...
		const freshness = new FreshnessManager({
			...
			onSyncComplete: (events, result) => {
				log(
					`Synced ${formatSyncResult(result)} from ${events.length} event(s)`,
				);
			},
			onSyncError: (events, error) => {
				log(
					`Sync failed for ${events.length} event(s): ${error instanceof Error ? error.message : String(error)}`,
				);
			},
		});
		if (freshness.start()) {
			...
			log(
				"Watcher unavailable; run `astrograph sync` manually or restart the daemon after filesystem support is fixed",
			);
		}
...
async function prepareIndex(
	...
	if (hasIndex) {
		log(
			`Existing index found: ${stats.data.fileCount} files, ${stats.data.nodeCount} symbols`,
		);
	...
	try {
		...
		reporter.done();
		log(
			formatInitReceipt(root, summaryFromStatus(freshStats.data)).replaceAll(
				"\n",
				" | ",
			),
		);
