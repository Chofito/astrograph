...
export async function runUninstallCore(
	args: string[],
	ctx: CliContext,
	homeDir: string,
): Promise<CliRunResult> {
	...
	for (const t of targets) {
		...
		const guideInstalled =
			guide === undefined ? false : isAgentGuideInstalled(guide, guideSource);
		if (!existsSync(path)) {
			plans.push({
				target: t,
				path,
	...
	const toWrite = plans.filter(
		(p) => !p.skip && (p.hasEntry || p.guideInstalled),
	);
	...
	if (toWrite.length === 0) {
		const lines = plans.map((p) => {
			...
			return style.dim(
				`${p.target.label}  ${style.path(p.path)}  not configured`,
			);
		});
	...
	for (const p of plans) {
		if (p.skip) {
			...
			const guideText = p.guideInstalled
				...
				: "not configured";
			planLines.push(
				`  ${style.dim(`${p.target.label}  ${style.path(p.path)}  ${guideText}`)}`,
			);
		} else {
			const action = p.guideInstalled
				? "remove astrograph + agent guide"
				: "remove astrograph";
			planLines.push(
				`  ${style.info(p.target.label)}  ${style.path(p.path)}  ${action}`,
			);
		}
	}
	planLines.push(
...
function formatGuideResult(
	...
	if (result.action === "removed")
		return `agent guide removed: ${style.path(result.path)}`;
