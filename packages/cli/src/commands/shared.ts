...
import { CliError, failOnPartial, ok, type CliContext, type CliRunResult } from '../cli';
...
export async function openGraphForRead<T>(
  ...
  return flags.failOnPartial === true ? failOnPartial(text, result.meta.partial) : ok(text);
}

export async function loadConfig(root: string): Promise<AstrographConfig | undefined> {
