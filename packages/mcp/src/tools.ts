...
export function createTools(session: ToolProjectSession): McpToolDefinition[] {
	return [
		tool(
			"astrograph_search",
			"Astrograph Search",
			"Find symbols by name across the indexed project.",
			searchSchema(),
			async (args) => {
				const input = parseSearch(args);
				return formatSearch(
					await callFacade(session, input.projectPath, (graph) =>
						graph.search(input),
					),
				);
			},
		...
		tool(
			"astrograph_context",
			"Astrograph Context",
			"Build ranked task context with related symbols and source blocks.",
			contextSchema(),
			async (args) => {
		...
		tool(
			"astrograph_trace",
			"Astrograph Trace",
			"Trace a call/reference path between two symbols.",
			traceSchema(),
			async (args) => {
		...
		tool(
			"astrograph_callers",
			"Astrograph Callers",
			"List project symbols that call a symbol.",
			callersSchema(),
			async (args) => {
				const input = parseCallers(args);
				return formatCallers(
					await callFacade(session, input.projectPath, (graph) =>
						graph.callers(input),
					),
				);
		...
		tool(
			"astrograph_callees",
			"Astrograph Callees",
			"List project symbols called by a symbol.",
			calleesSchema(),
			async (args) => {
				const input = parseCallees(args);
				return formatCallees(
					await callFacade(session, input.projectPath, (graph) =>
						graph.callees(input),
					),
				);
		...
		tool(
			"astrograph_impact",
			"Astrograph Impact",
			"Find symbols affected by changing a symbol.",
			impactSchema(),
			async (args) => {
		...
		tool(
			"astrograph_node",
			"Astrograph Node",
			"Show details and optional source for one symbol.",
			nodeSchema(),
			async (args) => {
				const input = parseNode(args);
				return formatNodeDetails(
					await callFacade(session, input.projectPath, (graph) =>
						graph.getNode(input),
					),
				);
		...
		tool(
			"astrograph_explore",
			"Astrograph Explore",
			"Return source blocks for related symbols grouped by file.",
			exploreSchema(),
			async (args) => {
				const input = parseExplore(args);
				return formatExplore(
					await callFacade(session, input.projectPath, (graph) =>
						graph.explore(input),
					),
				);
		...
		tool(
			"astrograph_status",
			"Astrograph Status",
			"Show index health, counts, backend, and coverage.",
			statusSchema(),
			async (args) => {
				const input = parseStatus(args);
				return formatStatus(
					await callFacade(session, input.projectPath, (graph) =>
						graph.getStats(input),
					),
				);
			},
		),
