...
import { empty, formatCodeBlock, formatEdge, formatNode, withBanner } from './shared';
...
export function formatContext(result: ToolResult<ContextOutput>): string {
  ...
  if (data.entryPoints.length > 0) {
    ...
    data.entryPoints.forEach((node, index) => lines.push(`${index + 1}. ${formatNode(node)}`));
  }
  ...
  lines.push('', `Stats: ${data.stats.nodeCount} nodes, ${data.stats.edgeCount} edges, ${data.stats.fileCount} files, ${data.stats.codeBlockCount} code blocks, ${data.stats.totalCodeChars} chars`);
