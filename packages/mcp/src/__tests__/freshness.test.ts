...
describe('MCP freshness manager', () => {
    ...
    const manager = new FreshnessManager({
      ...
      graph: fakeGraph({
        syncFiles: async (events) => {
          synced.push(events);
          return { added: [], modified: events.map((event) => event.path), removed: [] };
        },
      }),
      debounceMs: 10_000,
    });
  ...
  test('watcher-unavailable path marks answers partial without full-tree restat', async () => {
    ...
    try {
      ...
      expect(result.meta.partial).toBe(true);
      expect(result.meta.notes).toEqual(['watcher unavailable; freshness depends on explicit sync or pending events']);
    } finally {
      manager.close();
    }
  });
...
function fakeGraph(overrides: Partial<AstrographCore> = {}): AstrographCore {
  return {
    ...
    context: async () => ({ data: { entryPoints: [], subgraph: { nodes: [], edges: [] }, codeBlocks: [], inclusionReasons: {}, relatedFiles: [], stats: { nodeCount: 0, edgeCount: 0, fileCount: 0, codeBlockCount: 0, totalCodeChars: 0 } }, meta: resultWithMeta('').meta }),
    trace: async () => ({ data: { found: false, hops: [] }, meta: resultWithMeta('').meta }),
    ...
    getNode: async () => { throw new Error('not implemented'); },
