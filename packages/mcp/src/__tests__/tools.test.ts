...
describe("MCP tools", () => {
	...
	test("each tool returns agent text with an honest coverage banner", async () => {
		...
		const calls: [string, unknown, string][] = [
			...
			[
				"astrograph_node",
				{ symbol: "useAccount", includeCode: true },
	...
	test("missing index is returned as a tool error with init guidance", async () => {
		...
		try {
			const session = new ProjectSession({
				cwd: root,
				open: async () => fakeGraph() as never,
			});
			const result = await callTool(
				toolMap(createTools(session)),
				"astrograph_status",
				{},
			);
			...
			const graph = {
				...
				search: async () => ({
					data: [
						{
							node: node(
								synced ? "FreshSymbol" : "StaleSymbol",
								"src/fresh.ts",
								1,
								"function",
							),
	...
	test("server instructions steer agents toward Astrograph and coverage banners", () => {
		expect(SERVER_INSTRUCTIONS).toContain(
			"Astrograph is a pre-built local code graph",
		);
		expect(SERVER_INSTRUCTIONS).toContain(
			"Always check the final coverage banner",
		);
...
function fakeGraph(): AstrographCore {
	...
	return {
		search: async () => ({
			data: [
				{ node: node("makeNodeId", "src/ids.ts", 10, "function"), score: 2 },
			],
			meta: meta(),
		}),
		context: async () => ({
			data: {
				...
				codeBlocks: [
					block("src/account.ts", "export const useAccount = () => account;"),
				],
				inclusionReasons: {
					[useAccount.id]: "fts-match",
					[screen.id]: "called-by:useAccount",
				},
				...
				hops: [
					{
						...
						body: block(
							"src/screen.ts",
							"export const Screen = () => useAccount();",
						),
					},
				],
			},
		...
		callers: async () => ({
			data: [{ caller: screen, callSite: call }],
			meta: meta(),
		}),
		callees: async () => ({
			data: [{ callee: useAccount, callSite: call }],
			meta: meta(),
		}),
		impact: async () => ({
			data: [{ node: screen, distance: 1, viaPath: [call] }],
			meta: meta(),
		}),
		getNode: async () => ({
			data: {
		...
		getStats: async () => ({
			data: {
				...
				edgeCount: 1,
				fileCount: 1,
				nodesByKind: { function: 1, component: 1 },
				edgesByKind: { calls: 1 },
