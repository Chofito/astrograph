...
describe('MCP project session', () => {
  ...
  test('throws a clear init error when no index exists', async () => {
    ...
    try {
      const session = new ProjectSession({ cwd: root, open: async () => fakeGraph() as Astrograph });

      await expect(session.getGraph()).rejects.toBeInstanceOf(MissingIndexError);
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
...
function fakeGraph(overrides: Partial<AstrographCore> = {}): AstrographCore {
  return {
    ...
    context: async () => ({ data: { entryPoints: [], subgraph: { nodes: [], edges: [] }, codeBlocks: [], inclusionReasons: {}, relatedFiles: [], stats: { nodeCount: 0, edgeCount: 0, fileCount: 0, codeBlockCount: 0, totalCodeChars: 0 } }, meta: meta() }),
...
function meta() {
  return { coverage: { total: 0, resolved: 0, parsed: 0, pending: 0 }, partial: false };
}
